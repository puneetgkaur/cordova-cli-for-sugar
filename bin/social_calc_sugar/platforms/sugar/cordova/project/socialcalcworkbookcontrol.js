//Copyright  2010 SEETA
 
// All Rights Reserved. Legal, Privacy & Accessibility
//Co-developers: Anubhav Aggarwal, Shanjit Singh Jajmann
//Project Adviser: Manu Sheel Gupta 
//Special thanks to Rohit Jain for timely pointers.
//Contact e-mail - marketing<at>seeta<dot>in
//
...
SocialCalc.WorkBookControl = function(book, divid, defaultsheetname) {
	...
	this.sheetshtml = '<div id="fooBar" style="padding:0px;background-color:rgb(210, 210, 210);"></div>';

	this.buttonshtml = 
'<form>'+
'<div id="workbookControls" style="padding:0px;background-color:rgb(128, 128, 128);">'+
...
'<input type="button" value="Rename" onclick="SocialCalc.WorkBookControlRenameSheet()" class="smaller">'+
'<input type="button" value="Save" onclick="SocialCalc.WorkBookControlSaveSheet()" class="smaller">'+
//'<input type="button" value="new workbook" onclick="SocialCalc.WorkBookControlNewBook()" class="smaller">'+
'<input type="button" value="Load" onclick="SocialCalc.WorkBookControlLoad()" class="smaller">'+
'<input type="button" value="Export" onclick="SocialCalc.WorkBookExport()" class="smaller">'+
//'<input type="button" value="Export" onclick="SocialCalc.WorkBookExportOptions()" class="smaller">'+
//'<input type="button" value="Import" onclick="SocialCalc.WorkBookImport()" class="smaller">'+
'<input type="button" value="Exit" onclick="SocialCalc.WorkBookControlExit()" class="smaller">'+
...
SocialCalc.WorkBookImport = function()
{       
	var csvstring = ANDROID.sheetimport();
	ANDROID.testingstr("in javascript");
    var result = SocialCalc.ConvertOtherFormatToSave(csvstring,"csv");
	ANDROID.testingstr(result);
}
SocialCalc.WorkBookExportOptions = function()
{
	ANDROID.exportoptions();
}
SocialCalc.WorkBookExport = function()
{
SocialCalc.WorkBookControlSaveSheet();
var savestrme = "";
var name = "";
var saveme = ANDROID.loadme();

var sheetsaveme = JSON.parse(saveme);

for (var sheetme in sheetsaveme.sheetArr)
{   
    savestrme = sheetsaveme.sheetArr[sheetme].sheetstr.savestr ;
    name = sheetsaveme.sheetArr[sheetme].name;
}

var save_format_csv = SocialCalc.ConvertSaveToOtherFormat(savestrme,"csv");
//var save_format_xls = bla blah blah and then please use this in the arguments, any other ideas also rock.
ANDROID.testingstr(saveme);
ANDROID.sheetexport(save_format_csv,name);

}

SocialCalc.WorkBookControlExit = function()
{
ANDROID.exiting();	
}
...
SocialCalc.WorkBookControlDelSheet = function() {
	...
	for (var sheet in control.sheetButtonArr) {
		if (sheet != null) {
			...
			control.currentSheetButton.setAttribute("style","background-color:rgb(128, 128, 128)");
			break;
		}
	}
	if (control.currentSheetButton != null) {
...
SocialCalc.WorkBookControlAddSheet = function(addworksheet, sheetname){
	...
	element.setAttribute("style","background-color:rgb(128, 128, 128)");
...
SocialCalc.WorkBookControlActivateSheet = function(name) {
	...
	foo.setAttribute("style","background-color:rgb(120,120,120)");
...
SocialCalc.WorkBookControlSaveSheet = function(){
	ANDROID.testingstr("Enter saved");
	var control = SocialCalc.GetCurrentWorkBookControl();
	
	var sheetsave = {};
	sheetsave.numsheets = 1;
	...
	ANDROID.save(SocialCalc.TestWorkBookSaveStr);
...
SocialCalc.TestWorkBookSaveStr = ANDROID.loadme();
...
SocialCalc.WorkBookControlRenameSheet = function(){
   ...
   var str = '<div style="padding:6px 0px 4px 6px;">'+
         ...
         '<input type="text" id="newSheetName" style="width:280px;" value="'+'"><br>'+'</div>';

   str +='<div style="width:280px;text-align:right;padding:6px 0px 4px 6px;font-size:small;">'+
