...
SocialCalc.ParseSheetSave = function(savedsheet,sheetobj) {
   ANDROID.testingstr("Parse1");
   var lines=savedsheet.split(/\r\n|\n/);
   ANDROID.testingstr(savedsheet);
   var parts=[];
   var line;
   var i, j, t, v, coord, cell, attribs, name;
   var scc = SocialCalc.Constants;
   for (i=0;i<lines.length;i++) {
      ANDROID.testingstr("Parse2");
      line=lines[i];
      parts = line.split(":");
      switch (parts[0]) {
         case "cell":
   ...
   for (row = clipextents.cr1.row; row <= clipextents.cr2.row; row++) {
      for (col = clipextents.cr1.col; col <= clipextents.cr2.col; col++) {
         ...
         if (outputformat == "csv") {
            ...
            if (str.indexOf('"')!=-1) {
            ANDROID.testingstr("Shan was here");
...
SocialCalc.ConvertOtherFormatToSave = function(inputstr, inputformat) {
   ANDROID.testingstr("reached here convert to other to save");
   ...
   maxc = 0;
   if (inputformat == "csv") {
      ANDROID.testingstr("reached in csv changing");
      row = 0;
      inquote = false;
      for (i=0; i<lines.length; i++) {
